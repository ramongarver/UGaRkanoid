...
class Ground extends THREE.Object3D {
  ...
  constructor (scene, aWidth, aDeep, aMaterial, aBoxSize) {
    ...
    var geometriaSuelo = new THREE.BoxBufferGeometry (this.width, 0.2, this.deep, 1, 1, 1);
    ...
    if (pointOnGround !== null) {
      if (action === TheScene.NEW_BOX) {
        this.box = new THREE.Mesh (
          new THREE.BoxBufferGeometry (this.boxSize, this.boxSize, this.boxSize), 
          new THREE.MeshPhongMaterial ({color: Math.floor (Math.random()*16777215)}));
        this.box.geometry.translate (0, this.boxSize/2, 0);
