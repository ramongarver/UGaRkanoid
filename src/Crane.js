...
class Crane extends THREE.Object3D {
  ...
  createBase () {
    var geometriaBase = new THREE.CylinderBufferGeometry (this.craneWidth/10, this.craneWidth/10, this.baseHookHeight, 16, 1);
    geometriaBase.translate(0, this.baseHookHeight/2, 0);
    var base = new THREE.Mesh (geometriaBase,this.material);
  ...
  createMast () {
    var geometriaMast = new THREE.CylinderBufferGeometry (this.craneWidth/20, this.craneWidth/20, this.craneHeight, 16, 8);
    geometriaMast.translate(0, this.craneHeight/2, 0);
    var mast = new THREE.Mesh (geometriaMast, this.material);
  ...
  createJib () {
    var geometriaJib = new THREE.BoxBufferGeometry (this.craneWidth, this.craneWidth/10, this.craneWidth/10);
    geometriaJib.translate(0.3*this.craneWidth, this.craneWidth/20, 0);
    this.jib = new THREE.Mesh (geometriaJib, this.material);
  ...
  createTrolleyStringHook () {
    var geometriaTrolley = new THREE.BoxBufferGeometry (this.craneWidth/10, this.trolleyHeight, this.craneWidth/10);
    geometriaTrolley.translate(0, this.trolleyHeight/2, 0);
    this.trolley = new THREE.Mesh (geometriaTrolley, this.material);
    this.trolley.castShadow = true;
    this.trolley.position.y = -this.trolleyHeight;
    this.trolley.position.x = this.distanceMin;

    var geometriaString = new THREE.CylinderBufferGeometry (this.craneWidth/100, this.craneWidth/100, 1);
    geometriaString.translate(0, -0.5, 0);
    this.string = new THREE.Mesh (geometriaString, this.material);
    ...
    var geometriaHook = new THREE.CylinderBufferGeometry (this.craneWidth/40, this.craneWidth/40, this.baseHookHeight, 16, 1);
    geometriaHook.translate(0, -this.baseHookHeight/2, 0);
    this.hook = new THREE.Mesh (geometriaHook, this.material);
  ...
  update(guiControls) {
    this.setHookPosition (guiControls.rotation, guiControls.distance, guiControls.height);
  }
}
